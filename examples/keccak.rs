...
fn convert_bits_to_f<F: PrimeField<Repr = [u8; 32]>>(value_vec: &Vec<u8>) -> F {
    ...
    let mut sum_value_arr: Vec<u8> = (0..24).map(|t|{
        if t % 3 == 0 {
            value_vec[(t/3) * 8] + value_vec[(t/3) * 8 + 1] * 8 + (value_vec[(t/3) * 8 + 2] % 4) * 64  
        } else if t % 3 == 1 {
            value_vec[(t/3) * 8 + 2] / 4 + value_vec[(t/3) * 8 + 3] * 2 + (value_vec[(t/3) * 8 + 4]) * 16 +((value_vec[(t/3) * 8 + 5]) %2) * 128  
        } else {
            value_vec[(t/3) * 8 + 5] / 2  + value_vec[(t/3) * 8 + 6] * 4 + (value_vec[(t/3) * 8 + 7]) * 32  
        }
    }).collect();
...
fn eval_keccak_f_theta_step<F: PrimeField<Repr = [u8; 32]>>(
    ...
    for i in 0..PART_SIZE {
        ...
        for j in 0..PART_SIZE {
            absorb_rows.push((s[i][j], st_vec[j], s_new[i][j]));
        }
    }
...
fn eval_keccak_f_to_bit_vec<F: PrimeField<Repr = [u8; 32]>>(
    ...
    v1_vec.iter().zip(v2_vec.iter()).map(|(&v1, &v2)|(F::from_u128(v1 as u128), F::from_u128(v2 as u128))).collect()
} 
...
fn keccak_circuit<F: PrimeField<Repr = [u8; 32]> + Eq + Hash>(
    ...
    let keccak_first_step = ctx.step_type_def("keccak first step", |ctx| {
        ...
        let absorb_vec: Vec<Queriable<F>> = (0..PART_SIZE * PART_SIZE).map(|_|ctx.internal("absorb")).collect();
    ...
    let keccak_f_theta = ctx.step_type_def("keccak_f_theta_step", |ctx| {
        ...
        ctx.setup(move |ctx| {
            for i in 0..PART_SIZE {
                ctx.constr(eq(setup_s_vec[i][0] + setup_s_vec[i][1] + setup_s_vec[i][2] + setup_s_vec[i][3] + setup_s_vec[i][4], setup_sum_sum_split_value_vec[i]));

                for j in 0..PART_SIZE {
                    ctx.constr(eq(setup_sum_split_value_vec[i * PART_SIZE + j], setup_s_vec[i][j] + setup_sum_sum_split_xor_value_vec[(i + PART_SIZE - 1) % PART_SIZE] + setup_sum_sum_split_xor_move_value_vec[(i + 1) % PART_SIZE]));
                    ctx.transition(eq(setup_s_new_vec[i][j], setup_s_vec[i][j].next()))
                }
            }
            ctx.transition(eq(round, round.next()));
        ...
        ctx.wg::<(Vec<(F, F, F)>, Vec<(F, F)>, F), _>(move |ctx, (absorb_rows, sum_rows, round_value)| {
            for i in 0..PART_SIZE {
    ...
    let keccak_f_split_xor_vec: Vec<StepTypeWGHandler<F, (Vec<(F, F, F)>, Vec<(F, F)>, F), _>>  = (0..PART_SIZE * PART_SIZE).map(|s|
        ctx.step_type_def("keccak_f_split_xor_step", |ctx|{
            ...
            ctx.wg::<(Vec<(F, F, F)>, Vec<(F, F)>, F), _>(move |ctx, (absorb_rows, xor_rows, round_value)|{
                ctx.assign(coeff_eight, F::from_u128(8));
                let mut coeff_value = F::from_u128(1);
                for i in 0..NUM_BITS_PER_WORD {
                    ctx.assign(coeff_split_vec[i], coeff_value);
    ...
    let keccak_f_sum_split_xor_vec: Vec<StepTypeWGHandler<F, (Vec<(F, F, F)>, Vec<(F, F)>, F), _>> = (0..PART_SIZE).map(|s|
    ...
    let keccak_f_rho_move_vec: Vec<StepTypeWGHandler<F, (Vec<(F, F)>, Vec<F>, F), _>>= (0..PART_SIZE * PART_SIZE).map(|s|
        // a[x][y][z] = a[x][y][z-(t+1)(t+2)/2]
        ctx.step_type_def("keccak_f_rho_move_step", |ctx|{
            ...
            ctx.wg::<(Vec<(F, F)>, Vec<F>, F), _>(move |ctx, (absorb_rows, xor_rows, round_value)|{
                ctx.assign(coeff_eight, F::from_u128(8));
                let mut coeff_value = F::from_u128(1);
                for i in 0..NUM_BITS_PER_WORD {
                    ctx.assign(coeff_split_vec[i], coeff_value);
                ...
                for i in 0..NUM_BITS_PER_WORD {
                    ctx.assign(split_vec[i], xor_rows[i]);
                }
                ctx.assign(round, round_value);
            })
        })
    ...
    ctx.trace(move |ctx, params|{
        ...
        for (k, chunk) in chunks.enumerate() {
            let s = s_new.clone();
            let absorbs : Vec<F>= (0..PART_SIZE * PART_SIZE).map(|idx|{
                let i = idx % PART_SIZE;
                let j = idx / PART_SIZE;
            ...
            let absorb_rows: Vec<(F, F, F)> = (0..PART_SIZE * PART_SIZE).map(|idx|(s[idx / PART_SIZE][idx % PART_SIZE], absorbs[(idx % PART_SIZE) * PART_SIZE + (idx / PART_SIZE)], s_new[idx / PART_SIZE][idx % PART_SIZE])).collect();

            if k == 0{
                ctx.add(&keccak_first_step,(absorb_rows, F::ZERO));
            } else {
                let sum_rows: Vec<(F, F, F)> = (0..PART_SIZE * PART_SIZE).map(|i|{
                ...
                for t in 0..NUM_WORDS_TO_ABSORB {
                    let i = t % PART_SIZE;
                    let j = t / PART_SIZE;
                    let xor_rows = eval_keccak_f_to_bit_vec::<F>(sum_rows[i * PART_SIZE + j].1, sum_rows[i * PART_SIZE + j].2);
                    ctx.add(&keccak_f_split_xor_vec[i * PART_SIZE + j],(sum_rows.clone(), xor_rows, F::ZERO));
                }
            ...
            for round in 0..NUM_ROUNDS {
                ...
                for i in 0..PART_SIZE * PART_SIZE {
                    let xor_rows = eval_keccak_f_to_bit_vec::<F>(absorb_rows[i].1, absorb_rows[i].2);
                    ctx.add(&keccak_f_split_xor_vec[i],(absorb_rows.clone(), xor_rows, F::from(round as u64)));
                }
                let mut sum_rows = absorb_rows.clone();
                for i in 0..PART_SIZE {
                    let sum = absorb_rows[i * PART_SIZE].0 + absorb_rows[i * PART_SIZE + 1].0 + absorb_rows[i * PART_SIZE + 2].0 + absorb_rows[i * PART_SIZE + 3].0 + absorb_rows[i * PART_SIZE + 4].0;
                    let value = absorb_sum_rows[i].0;
                    let move_value =  absorb_sum_rows[i].1;
                    sum_rows.push((sum, value, move_value));
                }
                for i in 0..PART_SIZE {
                    let xor_rows = eval_keccak_f_to_bit_vec::<F>(sum_rows[i + PART_SIZE * PART_SIZE].0, sum_rows[i + PART_SIZE * PART_SIZE].1);
        ...
        for _ in 0..(params.output_len-1) / RATE_IN_BITS {
            ...
            let mut z_vec: Vec<F> = (0..NUM_WORDS_TO_ABSORB).map(|k| s_new[k % PART_SIZE][k / PART_SIZE]).collect();
            output.append(&mut z_vec); 
        }

        let mut output_bits: Vec<u8> = Vec::new();
        for i in 0..output.len() {
            let mut out_bits = convert_field_to_vec_bits(output[i]);
...
fn keccak_super_circuit<F: PrimeField<Repr = [u8; 32]> + Eq + Hash>(input_len: usize, output_len: usize) -> SuperCircuit<F, KeccakCircuit> {
    super_circuit::<F, KeccakCircuit, _>("keccak", |ctx| {
        ...
        let step_num = in_n * (1 + NUM_WORDS_TO_ABSORB + (NUM_ROUNDS ) * ((25 + 5 + 1) + (25) + 1 + (25 + 1) + 1)) - NUM_WORDS_TO_ABSORB + out_n * ((NUM_ROUNDS) * ((25 + 5 + 1) + (25) + 1 + (25 + 1) + 1)); 
