    ...
    for i in 0..PART_SIZE {
        for j in 0..PART_SIZE {
            ...
            let sum_split_vec = (0..NUM_PER_WORD)
                .map(|i| F::from_u128(sum_vec[2 * i] as u128 + sum_vec[2 * i + 1] as u128 * 8))
                .collect();
            let chi_split_vec = (0..NUM_PER_WORD)
                .map(|i| {
                    F::from_u128(
                        split_chi_value[2 * i] as u128 + split_chi_value[2 * i + 1] as u128 * 8,
                    )
                })
                .collect();
        ...
        ctx.setup(move |ctx| {
            ...
            for s in 0..NUM_WORDS_TO_ABSORB {
                ...
                let mut sum_split_vec: ASTExpr<F> =
                    setup_split_vec[s][0] * setup_coeff_split_vec[0];
                let mut sum_split_xor_vec: ASTExpr<F> =
                    setup_split_xor_vec[s][0] * setup_coeff_split_vec[0];
                for (k, &coeff) in setup_coeff_split_vec
                    .iter()
                    .enumerate()
                    .take(NUM_PER_WORD)
                    .skip(1)
                {
                    sum_split_vec = sum_split_vec + setup_split_vec[s][k] * coeff;
                    sum_split_xor_vec = sum_split_xor_vec + setup_split_xor_vec[s][k] * coeff;
                }
                ctx.constr(eq(sum_split_vec, setup_sum_split_value_vec[s]));
                ctx.constr(eq(sum_split_xor_vec, setup_s_new_vec[s]));
            }
            ...
            move |ctx, (absorb_rows, split_values, round_value)| {
                ...
                for i in 0..PART_SIZE {
                    for j in 0..PART_SIZE {
                        ...
                        ctx.assign(
                            s_new_vec[i * PART_SIZE + j],
                            absorb_rows[i * PART_SIZE + j].2,
                        );
                    }
                }
    ...
    let keccak_one_round = ctx.step_type_def("keccak one round", |ctx| {
        ...
        ctx.setup(move |ctx| {
            ...
            for s in 0..PART_SIZE {
                ...
                // 2. xor(sum)
                let mut sum_split_vec: ASTExpr<F> =
                    setup_theta_split_vec[s][0] * setup_coeff_split_vec[0];
                let mut sum_split_xor_vec: ASTExpr<F> =
                    setup_theta_split_xor_vec[s][0] * setup_coeff_split_vec[0];
                for (k, &coeff) in setup_coeff_split_vec
                    .iter()
                    .enumerate()
                    .take(NUM_PER_WORD)
                    .skip(1)
                {
                    sum_split_vec = sum_split_vec + setup_theta_split_vec[s][k] * coeff;
                    sum_split_xor_vec = sum_split_xor_vec + setup_theta_split_xor_vec[s][k] * coeff;
                }
                ...
                ctx.constr(eq(
                    setup_theta_bit_0[s] + setup_theta_bit_1[s] * 8,
                    setup_theta_split_xor_vec[s][NUM_PER_WORD - 1],
                ));
                let sum_split_xor_move_value_vec = (sum_split_xor_vec.clone()
            ...
            for i in 0..PART_SIZE {
                for j in 0..PART_SIZE {
                    ...
                    let mut theta_sum_split: ASTExpr<F> =
                        setup_theta_sum_split_vec[i * PART_SIZE + j][0] * setup_coeff_split_vec[0];
                    for (k, &coeff) in setup_coeff_split_vec
                        .iter()
                        .enumerate()
                        .take(NUM_PER_WORD)
                        .skip(1)
                    {
                        theta_sum_split = theta_sum_split
                            + setup_theta_sum_split_vec[i * PART_SIZE + j][k] * coeff;
                    }
                    ctx.constr(eq(
                        theta_sum_split,
                        setup_s_vec[i * PART_SIZE + j]
                    ...
                    if v % 2 != 0 {
                        rho_sum_split_move_vec = setup_rho_bit_1[i * PART_SIZE + j]
                            + setup_rho_bit_0[i * PART_SIZE + j]
                                * setup_coeff_split_vec[NUM_PER_WORD - 1]
                                * 8;
                        for k in 1..NUM_PER_WORD {
                            rho_sum_split_move_vec = rho_sum_split_move_vec
                                + setup_theta_sum_split_xor_vec[i * PART_SIZE + j]
                                    [(k - 1 + NUM_PER_WORD - (v - 1) / 2) % NUM_PER_WORD]
                                    * setup_coeff_split_vec[k - 1]
                                    * 8;
                        }
                        ctx.constr(eq(
                            setup_rho_bit_0[i * PART_SIZE + j]
                                * (setup_rho_bit_0[i * PART_SIZE + j] - 1),
                            0,
                        ));
                        ctx.constr(eq(
                            setup_rho_bit_1[i * PART_SIZE + j]
                                * (setup_rho_bit_1[i * PART_SIZE + j] - 1),
                            0,
                        ));
                        ctx.constr(eq(
                            setup_rho_bit_0[i * PART_SIZE + j]
                                + setup_rho_bit_1[i * PART_SIZE + j] * 8,
                            setup_theta_sum_split_xor_vec[i * PART_SIZE + j]
                                [(NUM_PER_WORD - (v + 1) / 2) % NUM_PER_WORD],
                        ));
                    } else {
                        rho_sum_split_move_vec = setup_theta_sum_split_xor_vec[i * PART_SIZE + j]
                            [(NUM_PER_WORD - v / 2) % NUM_PER_WORD]
                            * setup_coeff_split_vec[0];
                        for (k, &coeff) in setup_coeff_split_vec
                            .iter()
                            .enumerate()
                            .take(NUM_PER_WORD)
                            .skip(1)
                        {
                            rho_sum_split_move_vec = rho_sum_split_move_vec
                                + setup_theta_sum_split_xor_vec[i * PART_SIZE + j]
                                    [(k + NUM_PER_WORD - v / 2) % NUM_PER_WORD]
                                    * coeff;
                        }
                    }
                    ...
                    // a[y][2x + 3y] = a[x][y]
                    ctx.constr(eq(
                        rho_sum_split_move_vec,
                        setup_rho_pi_s_new_vec[j * PART_SIZE + ((2 * i + 3 * j) % PART_SIZE)],
                    ));
                }
            }
                ...
                for j in 0..PART_SIZE {
                    ...
                    for k in 0..NUM_PER_WORD {
                        ctx.add_lookup(
                            param
                                .chi_table
                                .apply(setup_chi_sum_split_value_vec[i * PART_SIZE + j][k])
                                .apply(setup_chi_split_value_vec[i * PART_SIZE + j][k]),
                        );
                    }

                    let mut sum_split_vec: ASTExpr<F> = setup_chi_sum_split_value_vec
                        [i * PART_SIZE + j][0]
                        * setup_coeff_split_vec[0];
                    let mut sum_split_chi_vec: ASTExpr<F> =
                        setup_chi_split_value_vec[i * PART_SIZE + j][0] * setup_coeff_split_vec[0];
                    for (k, &coeff) in setup_coeff_split_vec
                        .iter()
                        .enumerate()
                        .take(NUM_PER_WORD)
                        .skip(1)
                    {
                        sum_split_vec = sum_split_vec
                            + setup_chi_sum_split_value_vec[i * PART_SIZE + j][k] * coeff;
                        sum_split_chi_vec = sum_split_chi_vec
                            + setup_chi_split_value_vec[i * PART_SIZE + j][k] * coeff;
                    }
                    ctx.constr(eq(
                        sum_split_vec,
                        setup_chi_sum_value_vec[i * PART_SIZE + j],
                    ...
                    if i != 0 || j != 0 {
                        ...
                        for k in 1..NUM_PER_WORD {
                            sum_s_split_vec = sum_s_split_vec
                                + setup_final_sum_split_vec[k] * setup_coeff_split_vec[k];
                            sum_s_split_xor_vec = sum_s_split_xor_vec
                                + setup_final_xor_split_vec[k] * setup_coeff_split_vec[k];
                        }
        ...
        ctx.wg::<OneRoundValues<F>, _>(move |ctx, values| {
            ...
            for (q, v) in chi_sum_value_vec
                .iter()
                .zip(values.chi_sum_value_vec.iter())
            {
                ctx.assign(*q, *v)
            }
            for (q_vec, v_vec) in chi_sum_split_value_vec
                .iter()
                .zip(values.chi_sum_split_value_vec.iter())
            {
                for (q, v) in q_vec.iter().zip(v_vec.iter()) {
                    ctx.assign(*q, *v)
                }
...
fn keccak_super_circuit<F: PrimeField<Repr = [u8; 32]> + Eq + Hash>(
    ...
    super_circuit::<F, KeccakCircuit, _>("keccak", |ctx| {
        ...
        let maxwidth_config = config(MaxWidthCellManager::new(256, true), LogNSelectorBuilder {});
