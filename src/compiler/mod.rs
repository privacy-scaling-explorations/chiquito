...
use crate::{
    ...
    parser::{
        ast::{tl::TLDecl, DebugSymRef, Identifiable, Identifier},
        lang::TLDeclsParser,
    },
...
impl<F: Field + Hash> Compiler<F> {
    fn compile(&mut self, source: &str) -> Result<SBPIR<F, ()>, ()> {
    ...
    fn semantic(&mut self, ast: &[TLDecl<BigInt, Identifier>]) -> Result<SymTable, ()> {
        let result = semantic::analyser::analyse(ast);
    ...
    fn translate_query(
        ...
        match symbol.symbol.category {
            semantic::SymbolCategory::InputSignal
            | semantic::SymbolCategory::OutputSignal
            | semantic::SymbolCategory::InoutSignal => {
                self.translate_forward_queriable(machine_id, id)
            }
            semantic::SymbolCategory::Signal => match symbol.scope {
                semantic::ScopeCategory::Machine => {
                    self.translate_forward_queriable(machine_id, id)
                }
                semantic::ScopeCategory::State => {
                    if id.rotation() != 0 {
                ...
                semantic::ScopeCategory::Global => unreachable!("no global signals"),
            },

            semantic::SymbolCategory::State => {
                Queriable::StepTypeNext(self.get_step_type_handler(machine_id, &id.name()))
            }
