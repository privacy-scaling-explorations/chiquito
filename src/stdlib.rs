...
impl<F: FieldExt, const N_BYTES: usize> Lt<F, N_BYTES> {
    pub fn setup<V: Into<Constraint<F>>, StepArgs>(
        ctx: &mut StepTypeContext<F, StepArgs>,
        lhs: V,
        rhs: V,
        ...
        ctx.constr(lhs.expr - rhs.expr - expr_from_bytes(diff_bytes) + lt.clone() * Expr::from(range)); // constr takes Queriable or Expr or Constraint
