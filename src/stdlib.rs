...
impl<F: PrimeField, const N_BYTES: usize> Lt<F, N_BYTES> {
    pub fn setup<V: Into<Constraint<F>>, StepArgs>(
        ...
        let diff_bytes: Vec<Queriable<F>> = (0..N_BYTES).map(|idx| ctx.internal(format!("diff{idx}").as_str())).collect(); // might need to differentiate the annotation of different columns later
        ...
        ctx.constr(lhs.expr - rhs.expr - expr_from_bytes(diff_bytes.clone()) + lt.clone() * range.expr()); // constr takes Queriable or Expr or Constraint
