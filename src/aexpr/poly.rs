...
use crate::{ast::{query::Queriable, Expr, ToExpr, InternalSignal}, util::UUID};
...
fn compile_ir<F: Field>(result: &mut CompilationResult<F>, source: &AExprIR<F,Queriable<F>>) -> Expr<F> {
    use Expr::*;
...
fn compiler_ir_vec<F: Field>(result: &mut CompilationResult<F>, sub_expressions: &Vec<AExprIR<F,Queriable<F>>>) -> Vec<Expr<F>> {
    sub_expressions.iter().map(|se| compile_ir(result, se)).collect()
}

fn compile_mi<F: Field>(result: &mut CompilationResult<F>, source: &AExprIR<F,Queriable<F>> ) -> Expr<F> {
    use Expr::*;
    ...
    let virtual_constr = Mul(vec![virtual_eval_inverse.clone(), Sum(vec![Const(F::ONE), Neg(Box::new(Mul(vec![virtual_eval_inverse.clone(), virtual_signal.expr()])))])]);
    
    result.virtual_queriables.push(virtual_signal);
    result.virtual_assign_inv.insert(virtual_signal.uuid(), virtual_eval_inverse);
...
pub struct CompilationResult<F> {
    constr: Expr<F>,
    virtual_constr: Vec<Expr<F>>,
    virtual_queriables: Vec<Queriable<F>>,
    virtual_assign_inv: HashMap<UUID, Expr<F>>,
}

impl<F: Field> Default for CompilationResult<F> {
    fn default() -> Self {
        Self { constr: Expr::Const(F::ZERO), virtual_constr: Default::default(), virtual_queriables: Default::default(), virtual_assign_inv: Default::default() }
    }
}
...
mod test {
    ...
    use crate::{aexpr::ir::AExprIR::*, ast::{query::Queriable, InternalSignal}};
    ...
    fn test_simple_aexpr() {
        ...
        let input = Mul(vec![Query(a), MI(Box::new(Sum(vec![Query(b), Const(Fr::from(10))])))]);
